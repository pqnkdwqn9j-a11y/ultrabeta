...
local function MakeLangBtn(text, posX)
    ...
    b.Size = UD2(0,195,0,56); b.Position = UD2(0.5,posX,0.53,0)
    b.BackgroundColor3 = C3RGB(20,20,42); b.TextColor3 = C3(1,1,1)
...
if not (SavedKey and ValidKeys[SavedKey]) then
    ...
    IField.PlaceholderColor3=C3RGB(55,55,80); IField.TextColor3=C3RGB(220,220,255)
            ...
            if C and C.Silent and C.Silent.On and SilentTarget and SilentTarget.Parent then
                ...
                if method=="FireServer" or method=="InvokeServer" then
                    ...
                    for i,arg in ipairs(args) do
                        if typeof(arg)=="Vector3" then args[i]=pos; changed=true
                        elseif typeof(arg)=="CFrame" then args[i]=cf; changed=true end
                    end
...
local function SaveConfig()
    pcall(function()
        if writefile then
            local data = HttpService:JSONEncode(C)
            writefile(CFG_FILE, data)
        end
    end)
end
                ...
                if hum and hum.Health > 0 then
                    if not (C.Aim.Team and LP.Team and p.Team==LP.Team) then
                        ...
                        if part then
                            ...
                            if onS then
                                local dx=sv.X-cx; local dy=sv.Y-cy
...
local function ApplyHB(plr)
    ...
    if not HBData[plr] or HBData[plr].head ~= head then
        RestoreHB(plr)
        HBData[plr] = {head=head, origSize=head.Size, origTrans=head.Transparency, origMat=head.Material}
    end
end
...
local function CreateESP(plr)
    ...
    hl.FillColor=C3RGB(255,0,50); hl.FillTransparency=0.5
    ...
    bb.Adornee=head or hrp; bb.Size=UD2(0,200,0,80)
    bb.StudsOffset=V3(0,3.5,0); bb.AlwaysOnTop=true
    ...
    nl.TextSize=13; nl.Font=Enum.Font.GothamBold; nl.Text=plr.DisplayName
    ...
    ht.TextStrokeTransparency=0; ht.TextSize=11; ht.Font=Enum.Font.GothamMedium; ht.Text="100 HP"

    local dl=Instance.new("TextLabel",bb)
    dl.Size=UD2(1,0,0,13); dl.Position=UD2(0,0,0,40)
    dl.BackgroundTransparency=1; dl.TextColor3=C3RGB(180,180,255)
    dl.TextStrokeTransparency=0; dl.TextSize=11; dl.Font=Enum.Font.Gotham; dl.Text="0m"
        ...
        if t:IsA("Tool") then
            ...
            for _,d in ipairs(t:GetDescendants()) do
                if d:IsA("RemoteEvent") then
                    ...
                    if n:find("shoot") or n:find("fire") or n:find("attack") then tIns(evs,d) end
                end
            end
...
local function StartRapid()
    ...
    task.spawn(function()
        while RapidOn do
            ...
            DoFire(); task.wait(C.Gun.RapidSpeed/1000)
        end; RapidOn=false
    end)
end
...
-- ════ WATERMARK ════
local WMGui = Instance.new("ScreenGui", GetRoot())
WMGui.Name = "ULTRA_WM"
WMGui.IgnoreGuiInset = true
WMGui.DisplayOrder = 9999
WMGui.ResetOnSpawn = false

local WMLbl = Instance.new("TextLabel", WMGui)
WMLbl.Size = UD2(0,280,0,22)
WMLbl.Position = UD2(0,10,0,10)
WMLbl.BackgroundColor3 = C3RGB(8,8,18)
WMLbl.BackgroundTransparency = 0.3
WMLbl.BorderSizePixel = 0
...
task.spawn(function()
    while WMGui.Parent do
        ...
        WMLbl.Text = "ULTRA BETA | "..LP.DisplayName.." | "..fps.." FPS"
        WMLbl.Visible = C.WM.On
    end
end)
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if C.HB.On then
        ...
        if HBTimer>0.2 then HBTimer=0
            local sz=C.HB.Size; local sv=V3(sz,sz,sz)
            for plr,d in pairs(HBData) do
                if d.head and d.head.Parent then
                    if d.head.Size.X < sz-0.5 then
                        pcall(function()
                            d.head.Size=sv; d.head.Transparency=C.HB.Vis and 0.5 or 1
                            if C.HB.Vis then d.head.Material=Enum.Material.ForceField end
...
task.spawn(function()
    while task.wait(1.5) do
        if C.HB.On then
            for i=1,#PCache do if not HBData[PCache[i]] then ApplyHB(PCache[i]) end end
        end
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if DrawOK and C.ESP.On and C.ESP.Tracers then
            ...
            if hrp then
                ...
                if onS then
                    if not TracerLines[plr] then
                        pcall(function()
                            TracerLines[plr] = Drawing.new("Line")
                            TracerLines[plr].Color = C3RGB(255,255,0)
                            TracerLines[plr].Thickness = 1
                        end)
                    end
                    if TracerLines[plr] then
                        TracerLines[plr].From = bottom
                        TracerLines[plr].To = V2(sv.X, sv.Y)
        ...
        for _,line in pairs(TracerLines) do if line then line.Visible = false end end
    end
end)
        ...
        if C.ESP.On then
            ...
            for plr,data in pairs(ESPCache) do
                ...
                if not ch or not ch:FindFirstChildOfClass("Humanoid") or not ch:FindFirstChild("HumanoidRootPart") then
                    ...
                    if dist>C.ESP.Max or (C.ESP.Team and LP.Team and plr.Team==LP.Team) then
                        if data.hl then data.hl.Enabled=false end
                        if data.bb then data.bb.Enabled=false end
                    else
                        local pct=hum.Health/math.max(hum.MaxHealth,1)
                        if data.hl then data.hl.Enabled=C.ESP.Box
                            data.hl.FillColor=pct>0.6 and C3RGB(0,200,80) or (pct>0.3 and C3RGB(255,180,0) or C3RGB(255,40,40))
                        end
                        if data.bb then data.bb.Enabled=true
                            ...
                            if data.hpText then data.hpText.Visible=C.ESP.HP; data.hpText.Text=mFloor(hum.Health).."/"..mFloor(hum.MaxHealth).." HP" end
                            if data.distLbl then data.distLbl.Visible=C.ESP.Dist; data.distLbl.Text=mFloor(dist).."m" end
                        end
                    end
                end
            end
...
-- ════ МЕНЮ ════
local TH={
    bg=C3RGB(9,9,20),hdr=C3RGB(7,7,16),item=C3RGB(16,16,34),accent=C3RGB(0,170,255),
    text=C3RGB(215,215,238),dim=C3RGB(105,105,140),togOn=C3RGB(0,210,100),togOff=C3RGB(42,42,62),
...
Win.Size=UD2(0,540,0,520); Win.Position=UD2(0.5,-270,0.5,-260)
Win.BackgroundColor3=TH.bg; Win.BorderSizePixel=0; Win.Active=true; Win.Draggable=true
Instance.new("UICorner",Win).CornerRadius=UDim.new(0,12)
local WStroke=Instance.new("UIStroke",Win); WStroke.Color=TH.accent; WStroke.Thickness=1.5

task.spawn(function()
    local cols={TH.accent,C3RGB(130,50,255),C3RGB(255,50,100),TH.accent}; local i=1
    while MenuGui.Parent do TweenService:Create(WStroke,TweenInfo.new(2,Enum.EasingStyle.Sine),{Color=cols[i]}):Play(); i=i%#cols+1; task.wait(2) end
end)

local Hdr=Instance.new("Frame",Win); Hdr.Size=UD2(1,0,0,40); Hdr.BackgroundColor3=TH.hdr; Hdr.BorderSizePixel=0
Instance.new("UICorner",Hdr).CornerRadius=UDim.new(0,12)

local HT=Instance.new("TextLabel",Hdr); HT.Size=UD2(0,200,1,0); HT.Position=UD2(0,14,0,0)
HT.BackgroundTransparency=1; HT.Text="ULTRA BETA"; HT.TextColor3=TH.accent; HT.TextSize=18; HT.Font=Enum.Font.GothamBlack; HT.TextXAlignment=Enum.TextXAlignment.Left

local CB=Instance.new("TextButton",Hdr); CB.Size=UD2(0,28,0,28); CB.Position=UD2(1,-34,0.5,-14)
CB.BackgroundColor3=C3RGB(55,15,15); CB.TextColor3=C3(1,1,1); CB.Text="X"; CB.TextSize=13; CB.Font=Enum.Font.GothamBold; CB.BorderSizePixel=0
Instance.new("UICorner",CB).CornerRadius=UDim.new(0,6)
CB.MouseButton1Click:Connect(function() C.MenuOpen=false end)

local TB=Instance.new("Frame",Win); TB.Size=UD2(1,0,0,30); TB.Position=UD2(0,0,0,40); TB.BackgroundColor3=C3RGB(7,7,16); TB.BorderSizePixel=0
Instance.new("UIListLayout",TB).FillDirection=Enum.FillDirection.Horizontal

local Content=Instance.new("ScrollingFrame",Win); Content.Size=UD2(1,-6,1,-76); Content.Position=UD2(0,3,0,72)
Content.BackgroundTransparency=1; Content.BorderSizePixel=0; Content.ScrollBarThickness=3; Content.ScrollBarImageColor3=TH.accent
...
local function Sec(text)
    local f=Instance.new("Frame",Content); f.Size=UD2(1,-16,0,20); f.Position=UD2(0,8,0,Y); f.BackgroundTransparency=1
    ...
    lb.Size=UD2(0,0,1,0); lb.Position=UD2(0,4,0,0); lb.BackgroundColor3=TH.bg; lb.BorderSizePixel=0
    lb.Text="  "..text.."  "; lb.TextColor3=TH.sec; lb.TextSize=11; lb.Font=Enum.Font.GothamBold
    Y=Y+24
end

local function Tog(text,state,cb)
    local f=Instance.new("Frame",Content); f.Size=UD2(1,-16,0,30); f.Position=UD2(0,8,0,Y)
    ...
    dot.BackgroundColor3=C3(1,1,1); dot.BorderSizePixel=0; Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)
    local cur=state
...
local function Sld(text,mn,mx,def,cb)
    local f=Instance.new("Frame",Content); f.Size=UD2(1,-16,0,46); f.Position=UD2(0,8,0,Y)
    f.BackgroundColor3=TH.item; f.BorderSizePixel=0; Instance.new("UICorner",f).CornerRadius=UDim.new(0,7)
    ...
    local dr=false
    local sb=Instance.new("TextButton",sbg); sb.Size=UD2(1,0,3,4); sb.Position=UD2(0,0,-1,0); sb.BackgroundTransparency=1; sb.Text=""
    local function upd(mx2)
        local abs=sbg.AbsolutePosition; local sz=sbg.AbsoluteSize
        local rel=mClamp((mx2-abs.X)/sz.X,0,1); local val=mFloor(mn+(mx-mn)*rel)
...
local function Drp(text,opts,def,cb)
    ...
    btn.BackgroundColor3=C3RGB(24,24,46); btn.TextColor3=TH.accent; btn.TextSize=11; btn.Font=Enum.Font.GothamMedium
...
local function Btn(text,col,cb)
    local b=Instance.new("TextButton",Content); b.Size=UD2(1,-16,0,34); b.Position=UD2(0,8,0,Y)
...
Pages.Combat=function()
    Sec("AIMBOT")
    Tog(L.aimOn,C.Aim.On,function(v) C.Aim.On=v end)
    Drp(L.aMode,{"Hold","Toggle","Always"},C.Aim.Mode,function(v) C.Aim.Mode=v end)
    Drp(L.aPart,{"Head","HumanoidRootPart","Torso"},C.Aim.Part,function(v) C.Aim.Part=v end)
    Sld(L.aFOV,50,800,C.Aim.FOV,function(v) C.Aim.FOV=v end)
    Sld(L.aSmooth,1,20,C.Aim.Smooth,function(v) C.Aim.Smooth=v end)
    Sld(L.aPred,0,50,mFloor(C.Aim.Pred*100),function(v) C.Aim.Pred=v/100 end)
    Tog(L.aFovC,C.Aim.ShowFOV,function(v) C.Aim.ShowFOV=v end)
    Tog(L.aTeam,C.Aim.Team,function(v) C.Aim.Team=v end)
    Sec("SILENT AIM")
    Tog(L.silOn,C.Silent.On,function(v) C.Silent.On=v end)
    Drp(L.silPart,{"Head","HumanoidRootPart","Torso"},C.Silent.Part,function(v) C.Silent.Part=v end)
    Sec("TRIGGER BOT")
    Tog(L.trigOn,C.Trigger.On,function(v) C.Trigger.On=v end)
    Sec("HITBOX")
    Tog(L.hbOn,C.HB.On,function(v) C.HB.On=v; if v then for i=1,#PCache do ApplyHB(PCache[i]) end else CleanAllHB() end end)
    Sld(L.hbSz,5,30,C.HB.Size,function(v) C.HB.Size=v end)
    Tog(L.hbVis,C.HB.Vis,function(v) C.HB.Vis=v end)
end
...
Pages.Visual=function()
    Sec("ESP")
    Tog(L.espOn,C.ESP.On,function(v)
        C.ESP.On=v
        if v then for i=1,#PCache do if PCache[i].Character then CreateESP(PCache[i]) end end
        else CleanESP() end
    end)
    Tog(L.espBox,C.ESP.Box,function(v) C.ESP.Box=v end)
    Tog(L.espName,C.ESP.Name,function(v) C.ESP.Name=v end)
    Tog(L.espHP,C.ESP.HP,function(v) C.ESP.HP=v end)
    Tog(L.espDist,C.ESP.Dist,function(v) C.ESP.Dist=v end)
    Tog(L.espTracers,C.ESP.Tracers,function(v) C.ESP.Tracers=v end)
    Sld(L.espMax,100,5000,C.ESP.Max,function(v) C.ESP.Max=v end)
    Sec("CROSSHAIR")
    Tog(L.crossOn,C.Cross.On,function(v) C.Cross.On=v end)
    Sld(L.crossSz,5,30,C.Cross.Size,function(v) C.Cross.Size=v end)
    Sec("WORLD")
    Tog(L.fb,false,function(v) pcall(function()
        if v then Lighting.Brightness=3; Lighting.ClockTime=14; Lighting.FogEnd=1e9; Lighting.GlobalShadows=false; Lighting.Ambient=C3RGB(180,180,180)
        else Lighting.GlobalShadows=true; Lighting.Brightness=1; Lighting.Ambient=C3(0,0,0) end
    end) end)
end
...
Pages.Move=function()
    ...
    Tog(L.bhop,C.BHop.On,function(v) C.BHop.On=v end)
    Sec("FLY")
    Tog(L.flyOn,C.Fly.On,function(v) C.Fly.On=v; DoFly(v) end)
...
for i,k in ipairs(PageKeys) do
    ...
    tb.Size=UD2(1/#PageKeys,0,1,0); tb.BackgroundColor3=i==1 and TH.accent or C3RGB(10,10,20)
    tb.BackgroundTransparency=i==1 and 0 or 0.4; tb.TextColor3=C3(1,1,1); tb.TextSize=10
